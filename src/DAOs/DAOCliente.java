...
public class DAOCliente extends DAOGenerico<Cliente> {
    ...
    public List<Cliente> listByNome(String nome) {
        return em.createQuery("SELECT e FROM Cliente e WHERE e.rg LIKE :nome").setParameter("nome", "%" + nome + "%").getResultList();
    }
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getRg() + "-" + lf.get(i).getNomeCliente());
        }
        return ls;
    }
    ...
    public static void main(String[] args) {
        ...
        for (Cliente aluno : listaCliente) {
            System.out.println(aluno.getRg() + "-" + aluno.getNomeCliente());
        }
    }
}
