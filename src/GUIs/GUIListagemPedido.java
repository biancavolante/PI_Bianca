...
public class GUIListagemPedido extends JDialog {
    ...
    public GUIListagemPedido(List<Pedido> texto) {
        ...
        String[] colunas = new String[]{"IdPedido", "DataPedido", "ClienteRg", "FuncionarioId",};
        String[][] dados = new String[0][3];
        DefaultTableModel model = new DefaultTableModel(dados, colunas);
        JTable tabela = new JTable(model);
        scroll.setViewportView(tabela);
        for (int i = 0; i < texto.size(); i++) {
            String[] linha = new String[]{String.valueOf(texto.get(i).getIdPedido()), String.valueOf(texto.get(i).getDataPedido()), String.valueOf(texto.get(i).getClienteRg()), String.valueOf(texto.get(i).getFuncionarioId()),};
            model.addRow(linha);
        }
        painelTa.add(scroll);
